...
type GatewayClient = {
  ...
  close: () => void
  addEventListener: (handler: (event: GatewayEventFrame) => void) => void
  removeEventListener: (handler: (event: GatewayEventFrame) => void) => void
  addErrorListener: (handler: (error: Error) => void) => void
  removeErrorListener: (handler: (error: Error) => void) => void
  isClosed: () => boolean
}
...
function createGatewayClient(): GatewayClient {
  ...
  let closed = false
  let connected = false
  // Sets, not single slots: acquireGatewayClient shares one GatewayClient
  // across every concurrent /api/stream connection for the same key (see
  // sharedGatewayClients below). A single onEvent slot meant the most
  // recent caller's setOnEvent() silently overwrote every earlier
  // caller's handler -- any older-but-still-open SSE stream for that key
  // (a second browser tab, a not-yet-aborted reconnect, this file's own
  // earlier bug) would keep its connection open but never receive
  // another event, with no error to signal why. Confirmed live: an
  // isolated single-listener SSE capture on a session with zero other
  // concurrent connections received real per-token deltas fine; a
  // listener sharing a key with the app's own active stream received
  // nothing. Each acquire now gets its own listener; release removes
  // only that one.
  ...
  function addEventListener(handler: (event: GatewayEventFrame) => void) {
    eventListeners.add(handler)
  }

  function removeEventListener(handler: (event: GatewayEventFrame) => void) {
    eventListeners.delete(handler)
  }

  function addErrorListener(handler: (error: Error) => void) {
    errorListeners.add(handler)
  }

  function removeErrorListener(handler: (error: Error) => void) {
...
export async function acquireGatewayClient(
  ...
  if (existing && !existing.client.isClosed()) {
    existing.refs += 1
    if (options?.onEvent) existing.client.addEventListener(options.onEvent)
    if (options?.onError) existing.client.addErrorListener(options.onError)
    return {
      client: existing.client,
      release: function release() {
        if (options?.onEvent) existing.client.removeEventListener(options.onEvent)
        if (options?.onError) existing.client.removeErrorListener(options.onError)
  ...
  if (options?.onEvent) client.addEventListener(options.onEvent)
  if (options?.onError) client.addErrorListener(options.onError)
  await client.connect()
  sharedGatewayClients.set(key, { key, refs: 1, client })
  return {
    client,
    release: function release() {
      if (options?.onEvent) client.removeEventListener(options.onEvent)
      if (options?.onError) client.removeErrorListener(options.onError)
      releaseGatewayClient(key)
    },
  }
}
