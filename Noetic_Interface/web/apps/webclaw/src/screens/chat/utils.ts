...
export function textFromMessage(msg: GatewayMessage): string {
  // The Gateway sends user message content as a plain string, not the
  // array-of-parts shape the GatewayMessage type claims for all messages
  // (that shape is real for assistant messages, not user ones) - found by
  // comparing rendered (empty) user bubbles against the raw /api/history
  // response, which showed `"content": "hi"` for a user turn.
