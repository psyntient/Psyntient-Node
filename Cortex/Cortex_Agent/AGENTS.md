...
- Standing rule: For prior-work questions, use memory_search (and CURRENT.md) before broad file crawls. Semantic (vector) memory search is enabled (local embeddings, no cloud key) - trust ranked relevance, not just keyword hits, when using memory_search.
...
**Not sure what you can do?** Check `CAPABILITIES.md` before guessing or promising something you can't do - it's the living reference for every real capability, where it lives, and when to use it. Whenever you build, discover, or change a capability (new skill, script, workflow, config behavior), update `CAPABILITIES.md` in the same session - it's meant to stay accurate, not go stale.

## Model Tiering (OpenRouter nodes only)

If this Node's provider is OpenRouter, ordinary chat runs on a fast/cheap default model. Only escalate to a heavier model when the user explicitly invokes deep research or analysis - see the `research-agent` skill, which does this via `sessions_spawn({ model: ... })`. Never change `agents.defaults.model.primary` yourself for a single request - that's a Node-wide setting, not a per-request one.

If this Node's provider is NOT OpenRouter (direct Anthropic/OpenAI/etc.), this section doesn't apply - the provider's own default model already applies to every request and there's no OpenRouter-only tiering to reason about.
